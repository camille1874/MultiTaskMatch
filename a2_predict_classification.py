...
tf.app.flags.DEFINE_string("word2vec_model_path","../../GoogleNews-vectors-negative300.bin","word2vec's vocabulary and vectors") 
...
tf.app.flags.DEFINE_string("predict_target_file","../checkpoint_transformer/result_transformer_classification.csv","target file path for final prediction")
...
def main(_):
   ...
   # 3.create session.
    ...
    with tf.Session(config=config) as sess:
        ...
        logits=sess.run(model.return_logtis,feed_dict={model.input_x:testX2,model.input_y_label:testY2,model.dropout_keep_prob:1}) #logits:[batch_size,self.num_classes]
        answers = {}
        MAP, MRR = 0, 0
        total = len(logits)
