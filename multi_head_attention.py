...
class MultiHeadAttention(object):
    ...
    def multi_head_attention_fn(self):
        ...
        #dot_product=tf.reshape(dot_product,shape=(-1, length, h * d_k))
        #dot_product = self.channel_shuffle(dot_product, 4)
        # 4. linear projection
    ...
    def channel_shuffle(self, x, num_groups):
        ...
        #x_reshaped = tf.reshape(x, [-1, num_groups, h // num_groups, w, c])
        #x_transposed = tf.transpose(x_reshaped, [0, 2, 1, 3, 4])
        #output = tf.reshape(x_transposed, [-1, h, w, c])
        #return output
        n, w, c = x.shape.as_list()
        x_reshaped = tf.reshape(x, [-1, w, num_groups, c // num_groups])
        x_transposed = tf.transpose(x_reshaped, [0, 1, 3, 2])
        output = tf.reshape(x_transposed, [-1, w, c])
        return output
