...
def test(w, l2_reg, epoch, max_len, model_type, num_layers, data_type, word2vec, num_classes=2):
    if data_type == "WikiQA":
    ...
    for e in range(20, 22):
        test_data.reset_index()
        record_f = codecs.open("result" + data_type + str(e), encoding="utf-8", mode="w")
        ...
        with tf.Session() as sess:
            ...
            for i in range(test_data.data_size):
                ...
                if s1 in QA_pairs:
                    ...
                    cnt += 1
                record_f.write(str(cnt) + "\t" + s1 + "\t" + s2 + "\t" + str(pred) + "\n")
    ...
    exp_path = build_path("./experiments/", data_type, model_type, num_layers, ".txt")
    with codecs.open(exp_path, "w", encoding="utf-8") as f:
        f.write("Epoch\tMAP\tMRR\n")
        for i in range(len(MAPs)):
            f.write(str(i + 1) + "\t" + str(MAPs[i]) + "\t" + str(MRRs[i]) + "\n")
